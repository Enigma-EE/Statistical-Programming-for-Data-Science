...
for (day in 2:N_days) {
  ...
  # Calculate R0
  R0 <-
    sick_count * mean(mean_contacts) * infection_rate / N_population
  
  # Calculate new infections
  if (sick_count > 0) {
    contact_indices <- unlist(lapply(1:sick_count, function(i) {
      contacts <- sample(1:N_population, size = mean_contacts[i])
      contacts[runif(length(contacts)) < infection_rate * ifelse(sick_days[i] == 0, immunity_coefficient, 1) &
                 !sick_people[contacts]]
    }))
    ...
    new_infections <-
      0  # Updated to reflect no new infections if no sick people
  }
  ...
  simulation_results[day, "Contacts_Of_Sick"] <-
    ifelse(sick_count > 0, sum(mean_contacts[sick_people]), 0)
...
# Plot results
plot(
  simulation_results$Day,
  simulation_results$Sick_Count,
  type = "l",
  xlab = "Day",
...
lines(simulation_results$Day,
      simulation_results$Dead_Count,
      col = "red")
lines(simulation_results$Day,
      simulation_results$New_Infections,
      col = "navy")

legend(
  "topright",
  legend = c("Sick", "Dead", "New infections"),
...
cat('The number of sick people in the last day of the simulation is',
    simulation_results[N_days, "Sick_Count"],
    "\n")
cat('The number of dead people in the last day of the simulation is',
    simulation_results[N_days, "Dead_Count"],
    "\n")
...
cat('The mean of new infections in the simulation is',
    mean_new_infections,
    "\n")
